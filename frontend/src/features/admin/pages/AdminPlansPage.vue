...
<template>
  <div class="space-y-4">
    ...
    <DataTable :value="adminStore.plans" :loading="adminStore.loading" striped-rows>
      <Column field="name" header="Plan Name" />
      <Column field="tier" header="Tier" />
      <Column header="Monthly Price">
        <template #body="{ data }"> ${{ data.priceMonthly }} </template>
      </Column>
      <Column header="Yearly Price">
        <template #body="{ data }"> ${{ data.priceYearly }} </template>
      </Column>
      ...
      <Column header="Storage">
        <template #body="{ data }"> {{ data.maxStorageGb }} GB </template>
      </Column>
    </DataTable>
  </div>
</template>
