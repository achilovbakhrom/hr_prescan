...
export const interviewService = {
  ...
  async getInterviewDetail(id: string): Promise<InterviewDetail> {
    const response = await apiClient.get<InterviewDetail>(`/hr/interviews/${id}`)
    return response.data
  },

  async cancelInterview(id: string): Promise<Interview> {
    const response = await apiClient.post<Interview>(`/hr/interviews/${id}/cancel`)
    return response.data
  },

  async getObserverToken(id: string): Promise<{ token: string }> {
    const response = await apiClient.get<{ token: string }>(`/hr/interviews/${id}/observer-token`)
    return response.data
  },

  async resetInterview(interviewId: string): Promise<Interview> {
    const response = await apiClient.post<Interview>(`/hr/interviews/${interviewId}/reset/`)
    return response.data
  },

  // Candidate
  async getCandidateInterview(id: string): Promise<InterviewDetail> {
    const response = await apiClient.get<InterviewDetail>(`/candidate/interview/${id}`)
    return response.data
  },

  // Public — token-based interview access
  async getInterviewByToken(token: string): Promise<InterviewDetail> {
    const response = await apiClient.get<InterviewDetail>(`/public/interview/${token}/`)
    return response.data
  },

  async startInterview(token: string): Promise<InterviewDetail> {
    const response = await apiClient.post<InterviewDetail>(`/public/interview/${token}/start/`)
    return response.data
  },

  async sendChatMessage(token: string, message: string): Promise<ChatMessage> {
    const response = await apiClient.post<ChatMessage>(`/public/interview/${token}/chat/`, {
      message,
    })
    return response.data
  },
  ...
  async getChatHistory(token: string): Promise<ChatMessage[]> {
    const response = await apiClient.get<ChatMessage[]>(`/public/interview/${token}/chat/history/`)
    return response.data
  },
  ...
  async sendVoiceMessage(
    ...
    const response = await apiClient.post(`/public/interview/${token}/chat/voice`, formData, {
