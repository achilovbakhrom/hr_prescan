  ...
  <div class="space-y-4">
    <div class="flex items-center justify-between rounded-xl border border-gray-200 bg-gray-50 px-5 py-4">
      <div>
        ...
        <p class="mt-0.5 text-xs text-gray-400">{{ t('interviews.integrity.flagsDetected', { count: flags.length }) }}</p>
      </div>
      <div class="text-right">
        <span class="text-3xl font-bold" :class="scoreColor">{{ integrityScore }}</span>
        <span class="ml-1 text-sm text-gray-400">/100</span>
        <p class="mt-0.5 text-xs font-medium" :class="scoreColor">{{ t(`interviews.integrity.scoreLabels.${scoreLabel}`) }}</p>
      </div>
    </div>

    <p v-if="flags.length === 0" class="py-2 text-center text-sm text-gray-500">{{ t('interviews.integrity.noFlags') }}</p>
