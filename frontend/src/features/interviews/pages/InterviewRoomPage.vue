  ...
  <div class="relative flex h-screen flex-col bg-[#0a0d14]">
    ...
    <template v-else-if="room.interview.value">
      ...
      <RoomPreviewView
        ...
        :audio-devices="room.audioDevices.value"
        :video-devices="room.videoDevices.value"
        :selected-audio-device-id="room.selectedAudioDeviceId.value"
        :selected-video-device-id="room.selectedVideoDeviceId.value"
        :device-error="room.deviceError.value"
        :has-required-devices="room.hasRequiredDevices.value"
        :get-initials="room.getInitials"
        @toggle-mic="room.togglePreviewMic"
        @toggle-camera="room.togglePreviewCamera"
        @select-audio-device="room.selectAudioDevice"
        @select-video-device="room.selectVideoDevice"
      ...
      <RoomConnectedView
        ...
        :local-audio-level="room.localAudioLevel.value"
        :remote-audio-level="room.remoteAudioLevel.value"
        :local-is-speaking="room.localIsSpeaking.value"
        :remote-is-speaking="room.remoteIsSpeaking.value"
        :live-transcript="room.liveTranscript.value"
