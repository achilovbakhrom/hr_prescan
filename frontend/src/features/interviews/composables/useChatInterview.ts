...
export function useChatInterview(token: string) {
  ...
  function beforeUnloadHandler(e: BeforeUnloadEvent) {
    if (!isCompleted.value && messages.value.length > 0) e.preventDefault()
  }
  ...
  async function refreshStatus(): Promise<void> {
    try {
      if ((await interviewService.getInterviewByToken(token)).status === 'completed')
        isCompleted.value = true
    } catch {
      /* non-critical */
    }
  ...
  async function handleVoiceRecorded(blob: Blob, voiceDuration: number): Promise<void> {
    ...
    try {
      ...
      if (
        messages.value[li]?.role === 'candidate' &&
        messages.value[li]?.text === 'Transcribing...'
      )
        messages.value.pop()
      messages.value.push({
        role: 'ai',
