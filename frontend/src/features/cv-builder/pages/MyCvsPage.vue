...
const hasProfileData = computed(() => profile.value ? profile.value.completeness.score > 0 : false)
...
function viewCv(cv: CvFile): void { if (cv.downloadUrl) window.open(cv.downloadUrl, '_blank') }

function viewPublicCv(): void {
  if (profile.value?.shareToken) window.open(`${window.location.origin}/cv/${profile.value.shareToken}`, '_blank')
}
...
async function handleToggleActive(cv: CvFile): Promise<void> {
  actionLoading.value = cv.id; successMessage.value = null; error.value = null
  try {
    const result = await cvBuilderService.toggleCvActive(cv.id)
    if (result.isActive) { cvs.value.forEach((c) => { c.isActive = c.id === cv.id }) }
    else { cv.isActive = false }
...
function handleDelete(cv: CvFile): void {
  confirm.require({
    ...
    accept: async () => {
      actionLoading.value = cv.id; successMessage.value = null
      try { await cvBuilderService.deleteCv(cv.id); cvs.value = cvs.value.filter((c) => c.id !== cv.id); successMessage.value = t('myCvs.deleted') }
      catch { error.value = t('common.error') }
      finally { actionLoading.value = null }
    },
  })
...
<template>
  <div class="mx-auto max-w-3xl">
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <Button :label="t('myCvs.goToBuilder')" icon="pi pi-pencil" size="small" @click="router.push({ name: ROUTE_NAMES.CV_BUILDER })" />
    </div>
    ...
    <template v-else>
      ...
      <div v-if="!hasProfileData && cvs.length === 0" class="rounded-xl border border-dashed border-gray-200 py-16 text-center">
        ...
        <Button :label="t('myCvs.goToBuilder')" icon="pi pi-pencil" size="small" class="mt-4" @click="router.push({ name: ROUTE_NAMES.CV_BUILDER })" />
      </div>

      <div v-if="cvs.length > 0">
        <h2 class="mb-3 text-sm font-semibold uppercase tracking-wider text-gray-400">{{ t('myCvs.generatedPdfs') }}</h2>
        <div class="space-y-3">
          <CvFileCard v-for="cv in cvs" :key="cv.id" :cv="cv" :action-loading="actionLoading" @view="viewCv" @toggle-active="handleToggleActive" @delete="handleDelete" />
        </div>
      </div>
    </template>
