      ...
      <div class="min-w-0 flex-1">
        ...
        <div class="mt-1 flex flex-wrap items-center gap-2 text-xs text-gray-500">
          <span v-if="certification.issueDate"
            >{{ t('cvBuilder.certifications.issued') }}:
            {{ formatDisplayDate(certification.issueDate) }}</span
          >
          <span v-if="certification.expiryDate"
            >{{ t('cvBuilder.certifications.expires') }}:
            {{ formatDisplayDate(certification.expiryDate) }}</span
          >
        </div>
        <a
