...
const startDate = ref<Date | null>(props.editingExp?.startDate ? new Date(props.editingExp.startDate) : null)
const endDate = ref<Date | null>(props.editingExp?.endDate ? new Date(props.editingExp.endDate) : null)
...
function hasError(field: string): boolean { return field in props.fieldErrors }
function fieldError(field: string): string { return props.fieldErrors[field] ?? '' }
...
function setDescription(val: string): void { description.value = val }

defineExpose({ setDescription })
</script>

<template>
  <form class="mt-4 flex flex-col gap-4 rounded-lg border border-gray-200 p-4" @submit.prevent="handleSubmit">
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
      <div class="flex flex-col gap-1">
        <label for="expCompany" class="text-sm font-medium text-gray-700">{{ t('cvBuilder.experience.companyName') }} <span class="text-red-500">*</span></label>
      ...
      <div class="flex flex-col gap-1">
        <label for="expPosition" class="text-sm font-medium text-gray-700">{{ t('cvBuilder.experience.position') }} <span class="text-red-500">*</span></label>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
      <div class="flex flex-col gap-1">
        <label for="expType" class="text-sm font-medium text-gray-700">{{ t('cvBuilder.experience.employmentType') }}</label>
        <Select id="expType" v-model="employmentType" :options="employmentTypeOptions" optionLabel="label" optionValue="value" :placeholder="t('cvBuilder.experience.employmentTypePlaceholder')" class="w-full" :invalid="hasError('employmentType')" />
        <small v-if="hasError('employmentType')" class="text-red-500">{{ fieldError('employmentType') }}</small>
      </div>
      <div class="flex flex-col gap-1">
        <label for="expLocation" class="text-sm font-medium text-gray-700">{{ t('cvBuilder.experience.location') }}</label>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
      <div class="flex flex-col gap-1">
        <label for="expStartDate" class="text-sm font-medium text-gray-700">{{ t('cvBuilder.experience.startDate') }} <span class="text-red-500">*</span></label>
        <DatePicker id="expStartDate" v-model="startDate" dateFormat="yy-mm-dd" showIcon class="w-full" :invalid="hasError('startDate')" />
        <small v-if="hasError('startDate')" class="text-red-500">{{ fieldError('startDate') }}</small>
      </div>
      <div class="flex flex-col gap-1">
        <label for="expEndDate" class="text-sm font-medium text-gray-700">{{ t('cvBuilder.experience.endDate') }}</label>
        <DatePicker id="expEndDate" v-model="endDate" dateFormat="yy-mm-dd" showIcon :disabled="isCurrent" class="w-full" :invalid="hasError('endDate')" />
    ...
    <div class="flex items-center gap-2">
      ...
      <label for="expIsCurrent" class="text-sm text-gray-700">{{ t('cvBuilder.experience.currentlyWorking') }}</label>
    </div>

    <div class="flex flex-col gap-1">
      <div class="flex items-center justify-between">
        <label for="expDescription" class="text-sm font-medium text-gray-700">{{ t('cvBuilder.experience.description') }}</label>
        <Button :label="t('cvBuilder.improveWithAi')" icon="pi pi-sparkles" severity="secondary" text size="small" :loading="improvingDescription" :disabled="!description.trim()" @click="emit('improveDescription', description)" />
      </div>
