...
interface Props {
  ...
  value: string | number
  icon?: string
  iconAccent?: Accent | 'info' | 'teal' | 'success' | 'warning' | 'danger'
  deltaLabel?: string
  deltaDir?: DeltaDir
  sublabel?: string
  cardAccent?: Accent
...
const iconBgClass = computed(() => {
  switch (props.iconAccent) {
    ...
    case 'teal':
      return 'bg-[#ccfbf1] text-[#0d9488] dark:bg-[color:color-mix(in_srgb,#2dd4bf_18%,transparent)] dark:text-[#2dd4bf]'
    case 'success':
...
<template>
  ...
  >
    <div
      v-if="icon"
      class="flex h-[42px] w-[42px] shrink-0 items-center justify-center rounded-[--radius-sm]"
      :class="iconBgClass"
    >
    ...
    <div class="mt-5">
      <div
        class="font-mono text-[34px] font-semibold leading-none tracking-tight text-[color:var(--color-text-primary)]"
      >
        {{ value }}
      </div>
      <p class="mt-3 text-sm font-medium text-[color:var(--color-text-secondary)]">
        {{ label }}
      </p>
      <p
        v-if="deltaLabel"
        class="mt-1 flex items-center gap-1 font-mono text-xs"
        :class="deltaClass"
      >
        <span>{{ deltaLabel }}</span>
      </p>
      <p v-else-if="sublabel" class="mt-1 text-xs text-[color:var(--color-text-muted)]">
        {{ sublabel }}
      </p>
    </div>
  </GlassCard>
