...
<template>
  <div class="space-y-4 py-2">
    <div>
      ...
      <InputText v-model="title" class="w-full" :placeholder="t('vacancies.form.titlePlaceholder')" :invalid="props.hasError('title')" />
      <small v-if="props.hasError('title')" class="text-red-500">{{ props.fieldError('title') }}</small>
    </div>
    <div>
      <label class="mb-1 block text-sm font-medium">{{ t('vacancies.form.description') }} <span class="text-red-500">*</span></label>
      <Editor v-model="description" editorStyle="height: 200px" :class="{ 'border border-red-500 rounded-md': props.hasError('description') }" />
      <small v-if="props.hasError('description')" class="text-red-500">{{ props.fieldError('description') }}</small>
    </div>
    <div>
      <label class="mb-1 block text-sm font-medium">{{ t('vacancies.form.requirements') }}</label>
      <Textarea v-model="requirements" class="w-full" rows="3" :placeholder="t('vacancies.form.requirementsPlaceholder')" :invalid="props.hasError('requirements')" />
      <small v-if="props.hasError('requirements')" class="text-red-500">{{ props.fieldError('requirements') }}</small>
    </div>
    <div>
      <label class="mb-1 block text-sm font-medium">{{ t('vacancies.form.responsibilities') }}</label>
      <Textarea v-model="responsibilities" class="w-full" rows="3" :placeholder="t('vacancies.form.responsibilitiesPlaceholder')" :invalid="props.hasError('responsibilities')" />
      <small v-if="props.hasError('responsibilities')" class="text-red-500">{{ props.fieldError('responsibilities') }}</small>
    </div>
    <div>
      ...
      <small v-if="props.hasError('skills')" class="text-red-500">{{ props.fieldError('skills') }}</small>
    </div>

    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
      <div>
        <label class="mb-1 block text-sm font-medium">{{ t('vacancies.form.location') }}</label>
        <InputText v-model="location" class="w-full" :placeholder="t('vacancies.form.locationPlaceholder')" :invalid="props.hasError('location')" />
        <small v-if="props.hasError('location')" class="text-red-500">{{ props.fieldError('location') }}</small>
      </div>
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
      <div>
        ...
        <Dropdown v-model="employmentType" :options="employmentOptions" option-label="label" option-value="value" class="w-full" :invalid="props.hasError('employmentType')" />
        <small v-if="props.hasError('employmentType')" class="text-red-500">{{ props.fieldError('employmentType') }}</small>
      </div>
      <div>
        <label class="mb-1 block text-sm font-medium">{{ t('vacancies.form.experienceLevel') }}</label>
        <Dropdown v-model="experienceLevel" :options="experienceOptions" option-label="label" option-value="value" class="w-full" :invalid="props.hasError('experienceLevel')" />
        <small v-if="props.hasError('experienceLevel')" class="text-red-500">{{ props.fieldError('experienceLevel') }}</small>
      </div>
    </div>
    <div>
      <label class="mb-1 block text-sm font-medium">{{ t('vacancies.form.deadline') }}</label>
      <Calendar v-model="deadline" class="w-full md:w-1/2" date-format="yy-mm-dd" :show-icon="true" :invalid="props.hasError('deadline')" />
      <small v-if="props.hasError('deadline')" class="text-red-500">{{ props.fieldError('deadline') }}</small>
    </div>
    ...
    <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
      <div>
        ...
        <small v-if="props.hasError('salaryMin')" class="text-red-500">{{ props.fieldError('salaryMin') }}</small>
      </div>
      <div>
        ...
        <small v-if="props.hasError('salaryMax')" class="text-red-500">{{ props.fieldError('salaryMax') }}</small>
      </div>
      <div>
