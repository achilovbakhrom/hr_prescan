    ...
    <Column :header="t('candidates.prescanScore')" style="min-width: 80px">
      <template #body="{ data }">
        <span
          ...
          >{{ (data as Application).prescanningScore }}/10</span
        >
        <span v-else class="text-xs text-gray-400">---</span>
      </template>
    </Column>
    ...
    >
      <template #body="{ data }">
        <span
          ...
          >{{ (data as Application).interviewScore }}/10</span
        >
        <span v-else class="text-xs text-gray-400">---</span>
      </template>
    </Column>
    ...
    <Column header="" style="width: 160px" :exportable="false">
      <template #body="{ data }">
        <div class="flex items-center justify-end gap-1" @click.stop>
          <button
            v-if="
              (data as Application).status !== 'shortlisted' &&
              (data as Application).status !== 'hired' &&
          ...
          <button
            v-if="
              (data as Application).status !== 'rejected' &&
              (data as Application).status !== 'hired' &&
              (data as Application).status !== 'archived'
            "
