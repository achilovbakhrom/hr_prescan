...
<template>
    ...
    >
      ...
      <div class="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
        <div class="min-w-0 flex-1">
          ...
          <p v-if="(job as unknown as Record<string, unknown>).employerName || (job as unknown as Record<string, unknown>).companyName" class="mt-0.5 text-sm text-gray-500">
            <i class="pi pi-building mr-1 text-xs"></i>{{ (job as unknown as Record<string, unknown>).employerName || (job as unknown as Record<string, unknown>).companyName }}
          </p>
        </div>
        <span
  ...
  <div v-else class="flex flex-col items-center rounded-xl border border-dashed border-gray-200 bg-white py-12 text-center sm:py-20">
    ...
    <Button v-if="activeFilterCount > 0" :label="t('common.clearFilters')" text size="small" class="mt-3" @click="emit('clearFilters')" />
  </div>
</template>
