...
function openEdit(q: InterviewQuestion): void {
  isEditing.value = true; editId.value = q.id; formText.value = q.text; formCategory.value = q.category
  showDialog.value = true
}
...
function handleSubmit(): void {
  ...
  if (isEditing.value) {
    emit('update', editId.value, { text: formText.value.trim(), category: formCategory.value.trim() })
  } else {
      ...
      <Column :header="t('common.actions')" style="width: 120px">
        <template #body="{ data }">
          <div class="flex gap-1">
            ...
            <Button icon="pi pi-trash" severity="danger" text size="small" @click="emit('delete', data.id)" />
          </div>
        </template>
      </Column>
      <template #empty>
