...
<template>
  <div class="mb-4 rounded-xl border border-gray-200 bg-white p-4">
    <div class="flex items-center gap-4">
      <div class="flex h-16 w-16 shrink-0 items-center justify-center rounded-full" :class="scoreBg(matchScore)">
      ...
      <div class="flex-1">
        ...
        <TranslatableText
          ...
          :text="matchDetails.notes" :translations="matchNotesTranslations || {}"
          model="application" :object-id="applicationId" field="match_notes"
        >
          <template #default="{ text }"><p class="mt-0.5 text-xs text-gray-500">{{ text }}</p></template>
        </TranslatableText>
        <p v-else-if="matchDetails?.notes" class="mt-0.5 text-xs text-gray-500">{{ matchDetails.notes }}</p>
      </div>
    </div>

    <div v-if="matchDetails?.criteria_scores" class="mt-3 grid grid-cols-3 gap-2">
      <div v-for="(score, name) in matchDetails.criteria_scores" :key="name" class="rounded-lg bg-gray-50 p-2 text-center">
    ...
    <div v-if="matchDetails?.matching_skills?.length || matchDetails?.missing_skills?.length" class="mt-3 space-y-2">
      <div v-if="matchDetails?.matching_skills?.length">
        ...
        <div class="flex flex-wrap gap-1">
          <Tag v-for="s in matchDetails.matching_skills" :key="s" :value="s" severity="success" class="!text-[10px]" />
        </div>
      </div>
      <div v-if="matchDetails?.missing_skills?.length">
        ...
        <div class="flex flex-wrap gap-1">
          <Tag v-for="s in matchDetails.missing_skills" :key="s" :value="s" severity="danger" class="!text-[10px]" />
        </div>
      </div>
    </div>
  </div>
