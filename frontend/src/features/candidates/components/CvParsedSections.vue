...
const contacts = computed(() => {
  ...
  const iconMap: Record<string, string> = {
    email: 'pi-envelope', phone: 'pi-phone', location: 'pi-map-marker',
    linkedin: 'pi-linkedin', github: 'pi-github', website: 'pi-globe', telegram: 'pi-send',
  }
  return Object.entries(c).filter(([, v]) => v).map(([key, val]) => ({ label: key, value: val!, icon: iconMap[key] || 'pi-info-circle' }))
})
...
function contactHref(label: string, value: string): string | undefined {
  ...
  if (label === 'linkedin' || label === 'github' || label === 'website') return value.startsWith('http') ? value : `https://${value}`
  ...
  <div class="space-y-6">
    <div v-if="summary">
      ...
      <div class="rounded-lg border border-gray-100 bg-gray-50 p-3 text-sm text-gray-700">
        <TranslatableText v-if="applicationId" :text="summary" :translations="cvSummaryTranslations || {}" model="application" :object-id="applicationId" field="cv_summary">
          <template #default="{ text }"><div class="prose prose-sm max-w-none">{{ text }}</div></template>
        </TranslatableText>
    ...
    <div v-if="contacts.length">
      ...
      <div class="grid grid-cols-1 gap-2 md:grid-cols-2">
        <a v-for="c in contacts" :key="c.label" :href="contactHref(c.label, c.value)" target="_blank" rel="noopener" class="flex items-center gap-2.5 rounded-lg border border-gray-100 bg-gray-50 px-3 py-2 text-sm transition-colors hover:bg-gray-100" :class="contactHref(c.label, c.value) ? 'cursor-pointer' : 'cursor-default'">
    ...
    <div v-if="skills.length">
      ...
      <div class="flex flex-wrap gap-2"><Tag v-for="skill in skills" :key="skill" :value="skill" severity="info" /></div>
    </div>
    ...
    <div v-if="experience.length">
      ...
      <div class="space-y-3 border-l-2 border-blue-200 pl-4">
        <div v-for="(entry, idx) in experience" :key="idx" class="relative">
          ...
          <p class="text-sm text-gray-600">{{ entry.company }}<span v-if="entry.duration" class="text-gray-400"> &middot; {{ entry.duration }}</span></p>
    ...
    <div v-if="education.length">
      ...
      <div class="space-y-3">
        <div v-for="(entry, idx) in education" :key="idx" class="rounded border border-gray-100 bg-gray-50 p-3">
          <p class="font-medium">{{ entry.degree }}<template v-if="entry.field"> {{ t('candidates.cvData.inField') }} {{ entry.field }}</template></p>
          <p class="text-sm text-gray-600">{{ entry.institution }}<span v-if="entry.year" class="text-gray-400"> &middot; {{ entry.year }}</span></p>
        </div>
      </div>
    </div>
    ...
    <div v-if="languages.length">
      ...
      <div class="flex flex-wrap gap-2">
        <span v-for="(lang, idx) in languages" :key="idx" class="rounded-lg border border-gray-200 bg-gray-50 px-3 py-1.5 text-sm">
