    ...
    <div class="flex h-full w-full flex-col overflow-hidden bg-white sm:h-[85vh] sm:max-h-[700px] sm:max-w-3xl sm:rounded-2xl sm:shadow-2xl">
      <div class="flex items-center justify-between bg-gradient-to-r from-blue-600 to-blue-700 px-3 py-2.5 sm:px-4 sm:py-3">
        ...
        <div class="flex items-center gap-0.5 sm:gap-1">
          <Button icon="pi pi-external-link" severity="secondary" text rounded size="small" class="!h-8 !w-8 !text-white/70 hover:!bg-white/10 hover:!text-white sm:!h-9 sm:!w-9" @click="emit('openFullScreen')" />
          <Button icon="pi pi-minus" severity="secondary" text rounded size="small" class="!h-8 !w-8 !text-white/70 hover:!bg-white/10 hover:!text-white sm:!h-9 sm:!w-9" @click="emit('minimize')" />
        </div>
      </div>
  ...
  >
    <div class="mx-auto flex max-w-3xl items-center justify-between">
      ...
      <div class="flex items-center gap-1 sm:gap-2">
        ...
        <Button icon="pi pi-times" severity="secondary" text rounded size="small" class="!h-8 !w-8 sm:!h-9 sm:!w-9" @click.stop="emit('dismiss')" />
      </div>
    </div>
  </div>
</template>
